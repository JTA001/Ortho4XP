...
def build_poly_file(tile):
    ...
    # Let's go !
    UI.logprint("Step 1 for tile lat=", tile.lat,
                ", lon=", tile.lon, ": starting.")
    ...
    eps = 2**-5
    ortho_network = geometry.MultiLineString([geometry.LineString(
        [(x, 0.0-eps), (x, 1.0+eps)]) for x in xgrid]+[geometry.LineString([(0.0-eps, y), (1.0+eps, y)]) for y in ygrid])
...
def include_airports(vector_map, tile):
    ...
    tile.dem = DEM.DEM(tile.lat, tile.lon, tile.custom_dem,
                       tile.fill_nodata or "to zero", info_only=False)
...
def include_roads(vector_map, tile, apt_array, apt_area):
    def road_is_too_much_banked(way, filtered_segs):
        (col, row) = numpy.minimum(numpy.maximum(
            numpy.round(way[0]*1000), 0), 1000)
        if apt_array[int(1000-row), int(col)]:
            return True
        (col, row) = numpy.minimum(numpy.maximum(
            numpy.round(way[-1]*1000), 0), 1000)
        if apt_array[int(1000-row), int(col)]:
            return True
    ...
    if not road_network_banked.is_empty:
        ...
        timer = time.time()
        road_area = VECT.improved_buffer(road_network_banked.difference(VECT.improved_buffer(
            apt_area, tile.lane_width+2, 0, 0)), tile.lane_width, 2, 0.5, show_progress=True)
...
def include_sea(vector_map, tile):
    ...
    if os.path.isfile(custom_coastline):
        ...
        for osm_file in os.listdir(custom_coastline_dir):
            ...
            sea_layer.update_dicosm(os.path.join(
                custom_coastline_dir, osm_file), input_tags=None, target_tags=None)
    ...
    if not coastline.is_empty:
        ...
        vector_map.encode_MultiLineString(VECT.cut_to_tile(
            coastline, strictly_inside=True), tile.dem.alt_vec, 'SEA', check=True, refine=False)
        UI.vprint(3, "...done.")
        ...
        remainder = VECT.ensure_MultiLineString(VECT.cut_to_tile(geometry.MultiLineString(
            [line for line in coastline.geoms if not line.is_ring]), strictly_inside=True))
...
def include_water(vector_map, tile):
    large_lake_threshold = tile.max_area*1e6 / \
        (GEO.lat_to_m*GEO.lon_to_m(tile.lat+0.5))
        ...
        if (osmid in dicosmtags) and ('name' in dicosmtags[osmid]):
            if (dicosmtags[osmid]['name'] in good_imagery_list):
                UI.vprint(1, "      * ", dicosmtags[osmid]['name'],
                          "kept will complete imagery although it is", area, "km^2.")
                return False
            else:
                UI.vprint(1, "      * ", dicosmtags[osmid]['name'],
                          "will be masked like the sea due to its large area of", area, "km^2.")
                return True
        else:
            pt = pol.exterior.coords[0] if 'Multi' not in pol.geom_type else pol[0].exterior.coords[0]
            UI.vprint(1, "      * ", "Some large OSM water patch close to lat=", '{:.2f}'.format(
                pt[1]+tile.lon), "lon=", '{:.2f}'.format(pt[0]+tile.lat), "will be masked due to its large area of", area, "km^2.")
            return True
    ...
    if os.path.isfile(custom_water):
        ...
        for osm_file in os.listdir(custom_water_dir):
            UI.vprint(2, "      ", osm_file)
            water_layer.update_dicosm(os.path.join(
                custom_water_dir, osm_file), input_tags=None, target_tags=None)
    ...
    if not water_area.is_empty:
        ...
        try:
            ...
            return 0
        UI.vprint(2, "      Number of water Multipolygons : " +
                  str(len(dico_water)))
        UI.vprint(1, "      Encoding it.")
        vector_map.encode_MultiPolygon(dico_water, tile.dem.alt_vec, 'WATER', area_limit=tile.min_area /
                                       10000, simplify=tile.water_simplification*GEO.m_to_lat, check=True)
    if not sea_equiv_area.is_empty:
        ...
        vector_map.encode_MultiPolygon(dico_water, tile.dem.alt_vec, 'SEA_EQUIV', area_limit=tile.min_area /
                                       10000, simplify=tile.water_simplification*GEO.m_to_lat, check=True)
    return 1
##############################################################################
...
def include_patches(vector_map, tile):
    ...
    for pfile_name in os.listdir(patch_dir):
        ...
        try:
            patch_layer.update_dicosm(os.path.join(
                patch_dir, pfile_name), input_tags=None, target_tags=None)
        except:
        ...
        for wayid in waylist:
            way = numpy.array([dn[nodeid]
                              for nodeid in dw[wayid]], dtype=numpy.float)
            ...
            if wayid in dt['w']:
                ...
                if 'cst_alt_abs' in wtags:
                    alti_way = numpy.ones((len(way), 1)) * \
                        float(wtags['cst_alt_abs'])
                elif 'cst_alt_rel' in wtags:
                    alti_way = numpy.ones(
                        (len(way), 1))*(numpy.mean(tile.dem.alt_vec(way))+float(wtags['cst_alt_rel']))
                    ...
                    if len(way) != 5 or (way[0] != way[-1]).all():
                        UI.vprint(
                            1, "    Wrong number of nodes or non closed way for a altitude_high/altitude_low polygon, skipped.")
                        continue
                    short_high = way[-2:]
                    short_low = way[1:3]
                    try:
                    ...
                    if 'tanh' in rnw_profile:
                        def rnw_profile(x): return tanh_profile(alpha, x)
                    elif rnw_profile == 'spline':
                        rnw_profile = spline_profile
                    else:
                        rnw_profile = plane_profile
                    rnw_vect = (short_high[0]+short_high[1] -
                                short_low[0]-short_low[1])/2
                    rnw_length = sqrt(
                        rnw_vect[0]**2*cos(tile.lat*pi/180)**2+rnw_vect[1]**2)*111120
                    ...
                    if cuts_long:
                        ...
                        alti_way = numpy.array([altitude_high-rnw_profile(i/cuts_long)*(
                            altitude_high-altitude_low) for i in range(cuts_long+1)])
        ...
        for pfile_name in os.listdir(os.path.join(patch_dir, pdir_name)):
            ...
            try:
                ...
                try:
                    ...
                    continue
            patches_area = patches_area.union(keep_obj8(
                lat_anchor, lon_anchor, alt_anchor, heading_anchor, pfile_namelong, vector_map, tile))
    return (patches_area, patches_list)
##############################################################################
            ...
            try:
                ...
                for j in range(count//3):
                    ...
                    for (initp, endp) in ((a, b), (b, c), (c, a)):
                        vector_map.insert_edge(
                            initp, endp, vector_map.dico_attributes['INTERP_ALT'], check=True)
                    seed = (numpy.array(vector_map.nodes_dico[a])+numpy.array(
                        vector_map.nodes_dico[b])+numpy.array(vector_map.nodes_dico[c]))/3
                    if 'INTERP_ALT' in vector_map.seeds:
                        vector_map.seeds['INTERP_ALT'].append(seed)
                    else:
                        vector_map.seeds['INTERP_ALT'] = [seed]
