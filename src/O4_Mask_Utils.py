...
def build_masks(tile, for_imagery=False):
    ...
    if not os.path.exists(FNAMES.mesh_file(tile.build_dir, tile.lat, tile.lon)):
        UI.lvprint(0, "ERROR: Mesh file ", FNAMES.mesh_file(
            tile.build_dir, tile.lat, tile.lon), "absent.")
        UI.exit_message_and_bottom_line('')
        return 0
    dest_dir = FNAMES.mask_dir(tile.lat, tile.lon) if not for_imagery else os.path.join(
    ...
    for close_lat in range(tile.lat-1, tile.lat+2):
        for close_lon in range(tile.lon-1, tile.lon+2):
            close_build_dir = tile.build_dir if tile.grouped else tile.build_dir.replace(
                FNAMES.tile_dir(tile.lat, tile.lon), FNAMES.tile_dir(close_lat, close_lon))
    ...
    for mesh_file_name in mesh_file_name_list:
        ...
        for i in range(0, nbr_pt_in):
            pt_in[5*i:5*i+3] = [float(x)
                                for x in f_mesh.readline().split()[:3]]
        for i in range(0, 3):
            f_mesh.readline()
        for i in range(0, nbr_pt_in):
            pt_in[5*i+3:5*i+5] = [float(x)
        ...
        percent = -1
        UI.vprint(
            2, " Attribution process of masks buffers to water triangles for "+str(mesh_file_name)+".")
        for i in range(0, nbr_tri_in):
            ...
            if a == 0:
                if (til_x-16, til_y) in dico_masks:
                    dico_masks[(til_x-16, til_y)].append((lat1,
                                                          lon1, lat2, lon2, lat3, lon3))
                else:
                    dico_masks[(til_x-16, til_y)] = [(lat1,
                                                      lon1, lat2, lon2, lat3, lon3)]
                if b == 0:
                    if (til_x-16, til_y-16) in dico_masks:
                        dico_masks[(til_x-16, til_y-16)].append((lat1,
                                                                 lon1, lat2, lon2, lat3, lon3))
                    else:
                        dico_masks[(til_x-16, til_y-16)] = [(lat1,
                                                             lon1, lat2, lon2, lat3, lon3)]
                elif b == 3:
                    if (til_x-16, til_y+16) in dico_masks:
                        dico_masks[(til_x-16, til_y+16)].append((lat1,
            ...
            if b == 0:
                if (til_x, til_y-16) in dico_masks:
                    dico_masks[(til_x, til_y-16)].append((lat1,
                                                          lon1, lat2, lon2, lat3, lon3))
                else:
                    dico_masks[(til_x, til_y-16)] = [(lat1,
                                                      lon1, lat2, lon2, lat3, lon3)]
            elif b == 3:
                if (til_x, til_y+16) in dico_masks:
                    dico_masks[(til_x, til_y+16)].append((lat1,
    ...
    if tile.masks_use_DEM_too:
        try:
            ...
            source = ((";" in tile.custom_dem)
                      and tile.custom_dem.split(";")[0]) or tile.custom_dem
            tile.dem = DEM.DEM(tile.lat, tile.lon, source,
                               fill_nodata, info_only=False)
        except:
            UI.exit_message_and_bottom_line(
    ...
    def build_mask(til_x, til_y):
        ...
        if tile.masks_use_DEM_too:
            ...
            if demarr4326.any():
                ...
                demim3857 = demim3857.filter(ImageFilter.GaussianBlur(
                    0.3*2**(tile.mask_zl-14)))  # slight increase of area
        ...
        if tile.masks_custom_extent:
            ...
            masks_im = IMG.has_data(bbox_4326, tile.masks_custom_extent, True, mask_size=(
                4096, 4096), is_sharp_resize=False, is_mask_layer=False)
            if masks_im:
                custom_mask_array = (numpy.array(
                    masks_im, dtype=numpy.uint8)*(sea_level/255)).astype(numpy.uint8)

        # no need to test if the mask is all white since it would otherwise not be present in dico_mask
        if (img_array.max() == 0) and (custom_mask_array.max() == 0):
        ...
        img_array = numpy.maximum((img_array > 0).astype(
            numpy.uint8)*255, b_img_array)[1024:4096+1024, 1024:4096+1024]
...
def triangulation_to_image(name, pixel_size, grid_size_or_bbox):
    ...
    for i in range(0, nbr_pt):
        ...
        vertices[2*i:2*i+2] = [float(x)
                               for x in f_node.readline().split()[1:3]]
    f_node.close()
    xmin = vertices[::2].min()
    xmax = vertices[::2].max()
    ...
    mask_im = Image.new("1", (int((xmax-xmin)/pixel_size),
...
if __name__ == '__main__':
    ...
    pixel_size = pixel_size/111120 if epsg_code == '4326' else pixel_size
