...
def build_overlay(lat, lon):
    ...
    if not os.path.exists(file_to_sniff):
        UI.exit_message_and_bottom_line(
            "   ERROR: file ", file_to_sniff, "absent. Recall that the overlay source directory needs to be set in the config window first.")
        return 0
    ...
    if dsfid == '7z':
        ...
        os.system(unzip_cmd+' e -o'+FNAMES.Tmp_dir +
                  ' "'+file_to_sniff_loc+'.7z"')
    UI.vprint(1, "-> Converting the copy to text format")
    dsfconvertcmd = [dsftool_cmd.strip(), ' -dsf2text '.strip(), file_to_sniff_loc,
    ...
    f = open(os.path.join(FNAMES.Tmp_dir,
             FNAMES.short_latlon(lat, lon)+'_tmp_dsf.txt'), 'r')
    g = open(os.path.join(FNAMES.Tmp_dir, FNAMES.short_latlon(
        lat, lon)+'_tmp_dsf_without_mesh.txt'), 'w')
    line = f.readline()
    ...
    dsfconvertcmd = [dsftool_cmd.strip(), ' -text2dsf '.strip(), os.path.join(FNAMES.Tmp_dir, FNAMES.short_latlon(lat, lon) +
                                                                              '_tmp_dsf_without_mesh.txt'), os.path.join(FNAMES.Tmp_dir, FNAMES.short_latlon(lat, lon)+'_tmp_dsf_without_mesh.dsf')]
    fingers_crossed = subprocess.Popen(
        dsfconvertcmd, stdout=subprocess.PIPE, bufsize=0)
    while True:
        line = fingers_crossed.stdout.readline()
    ...
    if not os.path.exists(dest_dir):
        try:
            ...
            return 0
    shutil.copy(os.path.join(FNAMES.Tmp_dir, FNAMES.short_latlon(
        lat, lon)+'_tmp_dsf_without_mesh.dsf'), os.path.join(dest_dir, FNAMES.short_latlon(lat, lon)+'.dsf'))
    os.remove(os.path.join(FNAMES.Tmp_dir, FNAMES.short_latlon(
        lat, lon)+'_tmp_dsf_without_mesh.dsf'))
    os.remove(os.path.join(FNAMES.Tmp_dir, FNAMES.short_latlon(
        lat, lon)+'_tmp_dsf_without_mesh.txt'))
    os.remove(os.path.join(FNAMES.Tmp_dir,
              FNAMES.short_latlon(lat, lon)+'_tmp_dsf.txt'))
    os.remove(file_to_sniff_loc)
    try:
        os.remove(os.path.join(FNAMES.Tmp_dir, FNAMES.short_latlon(
            lat, lon)+'_tmp_dsf.txt.elevation.raw'))
        os.remove(os.path.join(FNAMES.Tmp_dir, FNAMES.short_latlon(
            lat, lon)+'_tmp_dsf.txt.sea_level.raw'))
    except:
        pass
    if dsfid == '7z':
