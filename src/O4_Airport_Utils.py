...
# as long as the chunks do not get smaller than this (in meters) .
chunk_min_size = 10
    ...
    for osmtype in ('r', 'w', 'n'):
        for osmid in (x for x in airport_layer.dicosmtags[osmtype] if 'aerodrome' in airport_layer.dicosmtags[osmtype][x].values() or 'airstrip' in airport_layer.dicosmtags[osmtype][x].values()):
            ...
            if 'name:en' in airport_layer.dicosmtags[osmtype][osmid]:
                name = airport_layer.dicosmtags[osmtype][osmid]['name:en'].replace(
                    "&quot;", '"').replace("&apos;", "'")
            elif 'name:alt' in airport_layer.dicosmtags[osmtype][osmid]:
                name = airport_layer.dicosmtags[osmtype][osmid]['name:alt'].replace(
                    "&quot;", '"').replace("&apos;", "'")
            elif 'name' in airport_layer.dicosmtags[osmtype][osmid]:
                name = airport_layer.dicosmtags[osmtype][osmid]['name'].replace(
                    "&quot;", '"').replace("&apos;", "'")
            else:
                name = '****'
            if len(name) >= 60:
            ...
            repr_node = airport_layer.dicosmn[osmid] if osmtype == 'n' else \
                ...
                tuple(numpy.mean(numpy.array([airport_layer.dicosmn[nodeid]
                      for nodeid in airport_layer.dicosmr[osmid]['outer'][0]]), axis=0))
            if not key:
            ...
            try:
                ...
                if dico_airports[key]['boundary'] and not dico_airports[key]['boundary'].is_valid:
                    UI.lvprint(
                        2, "Airport ", dico_airports[key], "OSM boundary is an invalid polygon, boundary set to None.")
                    dico_airports[key]['boundary'] = None
            except:
                UI.lvprint(2, "WARNING:  A presumably erroneous tag marked aerodrome was found and skipped close to the point",
...
def attach_surfaces_to_airports(airport_layer, dico_airports):
    ...
    for surface_type in ('runway', 'taxiway', 'apron', 'hangar'):
        for wayid in (x for x in airport_layer.dicosmw if x in airport_layer.dicosmtags['w'] and 'aeroway' in airport_layer.dicosmtags['w'][x] and airport_layer.dicosmtags['w'][x]['aeroway'] == surface_type):
            linestring = geometry.LineString(numpy.array(
                [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmw[wayid]]))
            found_apt = False
            ...
            closest_dist = 99999
            closest_apt = None
            pt_check = tuple(numpy.mean(numpy.array(
                [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmw[wayid]]), axis=0))
            ...
            if closest_apt and closest_dist < 3500:
                ...
                try:
                    ...
                    dico_airports[name] = {'key_type': 'name', 'repr_node': pt_check, 'name': name, 'runway': [
                    ], 'runway_as_rel': [], 'taxiway': [], 'apron': [], 'hangar': [], 'boundary': None}
                    dico_airports[name][surface_type].append(wayid)
                except:
                    dico_airports[pt_check] = {'key_type': 'repr_node', 'repr_node': pt_check, 'name': '****', 'runway': [
                    ], 'runway_as_rel': [], 'taxiway': [], 'apron': [], 'hangar': [], 'boundary': None}
    ...
    for relid in (x for x in airport_layer.dicosmr if x in airport_layer.dicosmtags['r'] and 'aeroway' in airport_layer.dicosmtags['r'][x] and airport_layer.dicosmtags['r'][x]['aeroway'] == 'runway'):
        linestring = geometry.LineString(numpy.array(
            [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmr[relid]['outer'][0]]))
        found_apt = False
        for airport in (x for x in dico_airports if dico_airports[x]['boundary']):
            if linestring.intersects(dico_airports[airport]['boundary']):
        ...
        closest_apt = None
        pt_check = tuple(numpy.mean(numpy.array(
            [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmr[relid]['outer'][0]]), axis=0))
        for airport in dico_airports:
            dist = GEO.dist(pt_check, dico_airports[airport]['repr_node'])
            if dist < closest_dist:
                closest_dist = dist
    ...
    for airport in dico_airports:
        ...
        for wayid in dico_airports[airport]['runway']:
            if airport_layer.dicosmw[wayid][0] == airport_layer.dicosmw[wayid][-1]:
                runway_pol = geometry.Polygon(numpy.round(numpy.array(
                    [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmw[wayid]])-numpy.array([tile.lon, tile.lat]), 7))
                if not runway_pol.is_empty and runway_pol.is_valid:
                    ...
                    if wayid not in airport_layer.dicosmtags['w'] or 'custom' not in airport_layer.dicosmtags['w'][wayid]:
                        discrep = runway_pol_rect.hausdorff_distance(
                            runway_pol)
                        if discrep > 0.0008:
                            UI.logprint("Bad runway (geometry too far from a rectangle) close to",
                                        airport, "at", dico_airports[airport]['repr_node'])
                            UI.vprint(1, "   !Bad runway (geometry too far from a rectangle) close to",
                                      airport, "at", dico_airports[airport]['repr_node'])
                            UI.vprint(1, "   !You may correct it editing the file ", FNAMES.osm_cached(
                                tile.lat, tile.lon, 'airports'), "in JOSM.")
                            continue
                    ...
                    UI.logprint(1, "Bad runway (geometry invalid or going back over itself) close to",
                                airport, "at", dico_airports[airport]['repr_node'])
                    UI.vprint(1, "   !Bad runway (geometry invalid or going back over itself) close to",
                              airport, "at", dico_airports[airport]['repr_node'])
                    UI.vprint(1, "   !You may correct it editing the file ", FNAMES.osm_cached(
                        tile.lat, tile.lon, 'airports'), "in JOSM.")
                    continue
        ...
        for relid in dico_airports[airport]['runway_as_rel']:
            runway_pol = geometry.Polygon(numpy.round(numpy.array(
                [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmr[relid]['outer'][0]])-numpy.array([tile.lon, tile.lat]), 7))
            if not runway_pol.is_empty and runway_pol.is_valid:
                if runway_pol.area < 1e-7:
                    # probably for aeromodelism only, we skip it.
                    continue
        ...
        while not runway_parts_are_grouped:
            ...
            for i in range(len(linear)-1):
                dir_i = numpy.arctan2(
                    *(numpy.array(airport_layer.dicosmn[linear[i][-1]])-numpy.array(airport_layer.dicosmn[linear[i][0]])))
                for j in range(i+1, len(linear)):
                    dir_j = numpy.arctan2(
                        *(numpy.array(airport_layer.dicosmn[linear[j][-1]])-numpy.array(airport_layer.dicosmn[linear[j][0]])))
                    ...
                    if linear[i][-1] == linear[j][0]:
                        linear = [linear[k] for k in range(len(linear)) if k not in (
                            i, j)]+[linear[i]+linear[j][1:]]
                        linear_width = [linear_width[k] for k in range(len(linear_width)) if k not in (
                            i, j)]+[max(linear_width[i], linear_width[j])]
                        runway_parts_are_grouped = False
                        break
                    elif linear[i][-1] == linear[j][-1]:
                        linear = [linear[k] for k in range(len(linear)) if k not in (
                            i, j)]+[linear[i]+linear[j][-2::-1]]
                        linear_width = [linear_width[k] for k in range(len(linear_width)) if k not in (
                            i, j)]+[max(linear_width[i], linear_width[j])]
                        runway_parts_are_grouped = False
                        break
        ...
        for (nodeid_list, width) in zip(linear, linear_width):
            ...
            runway_start = numpy.round(numpy.array(
                runway_start)-numpy.array([tile.lon, tile.lat]), 7)
            runway_end = numpy.round(numpy.array(
                runway_end)-numpy.array([tile.lon, tile.lat]), 7)
            if width:
                width += 10
            else:
                width = 30+runway_length//1000
            pol = geometry.Polygon(VECT.buffer_simple_way(
            ...
            for pol2 in runways_as_area:
                if (pol2[0].intersection(pol)).area > 0.6*min(pol.area, pol2[0].area):
                    ...
                    runways_as_area[i] = (
                        pol2[0], runway_start, runway_end, width)
        ...
        runway_area = VECT.ensure_MultiPolygon(ops.cascaded_union(
            [item[0] for item in runways_as_area+runways_as_line]))
        dico_airports[airport]['runway'] = (
            runway_area, runways_as_area, runways_as_line)
    return
####################################################################################################
...
def build_hangar_areas(tile, airport_layer, dico_airports):
    for airport in dico_airports:
        ...
        for wayid in wayid_list:
            try:
                pol = geometry.Polygon(numpy.round(numpy.array(
                    [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmw[wayid]])-numpy.array([[tile.lon, tile.lat]]), 7))
                if not pol.is_valid:
                    continue
            except:
                UI.vprint(2, "Unable to turn hangar area to polygon, close to",
...
def build_apron_areas(tile, airport_layer, dico_airports):
    for airport in dico_airports:
        ...
        for wayid in wayid_list:
            try:
                pol = geometry.Polygon(numpy.round(numpy.array(
                    [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmw[wayid]])-numpy.array([[tile.lon, tile.lat]]), 7))
                if not pol.is_valid:
                    UI.vprint(2, "Unable to turn apron area to polygon, close to",
                              airport_layer.dicosmn[airport_layer.dicosmw[wayid][0]])
                    continue
            except:
                UI.vprint(2, "Unable to turn apron area to polygon, close to",
        ...
        dico_airports[airport]['apron'] = (
            aprons, dico_airports[airport]['apron'])
    return
####################################################################################################
...
def build_taxiway_areas(tile, airport_layer, dico_airports):
    for airport in dico_airports:
        ...
        taxiways = geometry.MultiLineString([geometry.LineString(numpy.round(numpy.array(
            [airport_layer.dicosmn[nodeid] for nodeid in airport_layer.dicosmw[wayid]])-numpy.array([[tile.lon, tile.lat]]), 7)) for wayid in wayid_list])
        taxiways = VECT.ensure_MultiPolygon(
            VECT.improved_buffer(taxiways, 15, 3, 0.5))
        dico_airports[airport]['taxiway'] = (
            taxiways, dico_airports[airport]['taxiway'])
    return
####################################################################################################
...
def update_airport_boundaries(tile, dico_airports):
    for airport in dico_airports:
        ...
        if apt['boundary']:
            apt['boundary'] = VECT.ensure_MultiPolygon(ops.cascaded_union([affinity.translate(
                apt['boundary'], -tile.lon, -tile.lat), boundary]).buffer(0).simplify(0.00001))
        else:
...
def list_airports_and_runways(dico_airports):
    ...
    airport_list = sorted([x for x in dico_airports if dico_airports[x]['key_type'] == 'icao']) +\
        ...
        sorted([x for x in dico_airports if dico_airports[x]
               ['key_type'] == 'repr_node'])
    for airport in airport_list:
        l = len(dico_airports[airport]['runway'][1]) + \
            len(dico_airports[airport]['runway'][2])
        runway_str = str(l)+(' runways,' if l >
                             1 else ' runway ,') if l else 'boundary ,'
        if dico_airports[airport]['key_type'] in ('icao', 'iata', 'local_ref'):
            UI.vprint(1, '  ', '{:6s}'.format(airport), '{:60s}'.format(dico_airports[airport]['name']), runway_str, 'lat=', '{:.2f}'.format(
                dico_airports[airport]['repr_node'][1])+',', 'lon=', '{:.2f}'.format(dico_airports[airport]['repr_node'][0]))
        else:
            UI.vprint(1, '  ', '{:6s}'.format('****'), '{:60s}'.format(dico_airports[airport]['name']), runway_str, 'lat=', '{:.2f}'.format(
                dico_airports[airport]['repr_node'][1])+',', 'lon=', '{:.2f}'.format(dico_airports[airport]['repr_node'][0]))
    return
####################################################################################################
...
def smooth_raster_over_airports(tile, dico_airports, preserve_boundary=True):
    ...
    for airport in dico_airports:
        try:
            pix = int(dico_airports[airport]['smoothing_pix']
                      ) if 'smoothing_pix' in dico_airports[airport] else tile.apt_smoothing_pix
        except:
        ...
        full_area = VECT.ensure_MultiPolygon(ops.cascaded_union([dico_airports[airport]['boundary'], dico_airports[airport]['runway']
                                             [0], dico_airports[airport]['hangar'], dico_airports[airport]['taxiway'][0], dico_airports[airport]['apron'][0]]))
        for polygon in full_area:
            exterior_pol_pix = [(round(upscale*(X-X0)/xstep), round(upscale*(Y1-Y)/ystep))
                                for (X, Y) in polygon.exterior.coords]
            airport_draw.polygon(exterior_pol_pix, fill='white')
            for inner_ring in polygon.interiors:
                interior_pol_pix = [(round(
                    upscale*(X-X0)/xstep), round(upscale*(Y1-Y)/ystep)) for (X, Y) in inner_ring.coords]
        ...
        tile.dem.alt_dem[rowmin:rowmax+1, colmin:colmax+1] = DEM.smoothen(
            tile.dem.alt_dem[rowmin:rowmax+1, colmin:colmax+1], pix, airport_im, preserve_boundary=False)
    if preserve_boundary:
        pix = max_pix
        for i in range(pix):
            tile.dem.alt_dem[i] = i/pix*tile.dem.alt_dem[i]+(pix-i)/pix*up[i]
            tile.dem.alt_dem[-i-1] = i/pix * \
                tile.dem.alt_dem[-i-1]+(pix-i)/pix*down[-i-1]
        for i in range(pix):
            tile.dem.alt_dem[:, i] = i/pix * \
                tile.dem.alt_dem[:, i]+(pix-i)/pix*left[:, i]
            tile.dem.alt_dem[:, -i-1] = i/pix * \
                tile.dem.alt_dem[:, -i-1]+(pix-i)/pix*right[:, -i-1]
    tile.dem.write_to_file(FNAMES.alt_file(tile))
    return
####################################################################################################
...
def encode_runways_taxiways_and_aprons(tile, airport_layer, dico_airports, vector_map, patches_list):
    ...
    for airport in dico_airports:
        ...
        for wayid in apt['taxiway'][1]:
            taxiway = numpy.array([airport_layer.dicosmn[nodeid]
                                  for nodeid in airport_layer.dicosmw[wayid]])-numpy.array([[tile.lon, tile.lat]])
        ...
        for (runway_pol, runway_start, runway_end, runway_width) in []:
            ...
            for pol in VECT.ensure_MultiPolygon(VECT.cut_to_tile(runway_pol)):
                way = numpy.round(VECT.refine_way(
                    numpy.array(pol.exterior.coords), refine_size), 7)
                ...
                vector_map.insert_way(numpy.hstack(
                    [way, alti_way]), 'RUNWAY', check=True)
                pols.append(pol)
            way = VECT.refine_way(numpy.vstack(
                (runway_start, runway_end)), refine_size)
            ...
            for k in range(1, len(way)):
                try:
                    lin = geometry.LineString(
                        [way_r[k], way_l[k]]).intersection(runway_pol)
                    if lin.geom_type == "LineString":
                        trav = numpy.round(numpy.array(lin), 7)
                        alti_trav = numpy.array([VECT.weighted_alt(
                            node, alt_idx, alt_dico, tile.dem) for node in trav]).reshape((len(trav), 1))
                        vector_map.insert_way(numpy.hstack(
        ...
        for (runway_pol, runway_start, runway_end, runway_width) in apt['runway'][1]+apt['runway'][2]:
            ...
            for pol in VECT.ensure_MultiPolygon(VECT.cut_to_tile(runway_pol)):
                ...
                abscissae = [boundary.project(geometry.Point(x))
                             for x in boundary.coords]
                traverses = []
                for k in range(1, len(way)):
                    try:
                        lin = geometry.LineString(
                ...
                way = numpy.round(numpy.array(
                    [boundary.interpolate(x).coords[0] for x in abscissae+[0]]), 7)
                alti_way = numpy.array([VECT.weighted_alt(
                    node, alt_idx, alt_dico, tile.dem) for node in way]).reshape((len(way), 1))
                vector_map.insert_way(numpy.hstack(
                    [way, alti_way]), 'RUNWAY', check=True)
                for (abs1, abs2) in traverses:
                    trav = numpy.round(numpy.array([boundary.interpolate(
                        abs1).coords[0], boundary.interpolate(abs2).coords[0]]), 7)
                    alti_trav = numpy.array([VECT.weighted_alt(
                        node, alt_idx, alt_dico, tile.dem) for node in trav]).reshape((len(trav), 1))
                    vector_map.insert_way(numpy.hstack(
                        [trav, alti_trav]), 'DUMMY', check=True)
        ...
        cleaned_taxiway_area = VECT.improved_buffer(apt['taxiway'][0].difference(VECT.improved_buffer(
            apt['runway'][0], 5, 0, 0).union(VECT.improved_buffer(apt['hangar'], 20, 0, 0))), 3, 2, 0.5)
        # update it
            ...
            try:
                ...
                if not apron_pol.is_empty and runway_pol.is_valid:
                    alti_way = numpy.array([VECT.weighted_alt(
                        node, alt_idx, alt_dico, tile.dem) for node in way]).reshape((len(way), 1))
                    vector_map.insert_way(numpy.hstack(
                        [way, alti_way]), 'APRON', check=True)
                    seeds['APRON'].append(numpy.array(
                        apron_pol.representative_point()))
            except:
                pass
    ...
    UI.vprint(1, "   Auto-patched", total_rwy, "runway"+plural_rwy +
              " and", total_taxi, "piece"+plural_taxi+" of taxiway.")
...
def flatten_helipads(airport_layer, vector_map, tile, treated_area):
    ...
    for wayid in (x for x in airport_layer.dicosmw if x in airport_layer.dicosmtags['w'] and 'aeroway' in airport_layer.dicosmtags['w'][x] and airport_layer.dicosmtags['w'][x]['aeroway'] == 'helipad'):
        if airport_layer.dicosmw[wayid][0] != airport_layer.dicosmw[wayid][-1]:
            continue
        way = numpy.round(numpy.array([airport_layer.dicosmn[nodeid]
                          for nodeid in airport_layer.dicosmw[wayid]])-numpy.array([[tile.lon, tile.lat]]), 7)
        pol = geometry.Polygon(way)
        if (pol.is_empty) or (not pol.is_valid) or (not pol.area) or (pol.intersects(treated_area)):
            continue
        multipol.append(pol)
    ...
    for nodeid in (x for x in airport_layer.dicosmn if x in airport_layer.dicosmtags['n'] and 'aeroway' in airport_layer.dicosmtags['n'][x] and airport_layer.dicosmtags['n'][x]['aeroway'] == 'helipad'):
        center = numpy.round(numpy.array(
            airport_layer.dicosmn[nodeid])-numpy.array([tile.lon, tile.lat]), 7)
        if geometry.Point(center).intersects(helipad_area) or geometry.Point(center).intersects(treated_area):
            continue
        way = numpy.round(center+numpy.array([[cos(k*pi/3)*9*GEO.m_to_lon(
            tile.lat), sin(k*pi/3)*9*GEO.m_to_lat] for k in range(7)]), 7)
        pol = geometry.Polygon(way)
        multipol.append(pol)
        # alti_way=numpy.ones((len(way),1))*numpy.mean(tile.dem.alt_vec(way))
        # vector_map.insert_way(numpy.hstack([way,alti_way]),'INTERP_ALT',check=True)
