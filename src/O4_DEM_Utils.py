...
import array
import numpy
# import gdal from osgeo instead of directly (2022-07-06)
from osgeo import gdal
has_gdal = TRUE             # gdal is present (2022-07-06)
...
class DEM():
    def __init__(self, lat, lon, source='', fill_nodata=True, info_only=False):
        ...
        if fill_nodata == "to zero":
            ...
            if not fill_nodata_values_with_nearest_neighbor(self.alt_dem, self.nodata):
                UI.vprint(
                    1, "   INFO: Dataset contains too much no_data to be filled.")
                self.nodata_to_zero()

        UI.vprint(1, "    * Min altitude:", self.alt_dem.min(),
                  ", Max altitude:", self.alt_dem.max(), ", Mean:", self.alt_dem.mean())
    ...
    def load_data(self, source, info_only=False):
        ...
        if source in available_sources[1::2]:
            ...
            if short_source in global_sources:
                (self.epsg, self.x0, self.y0, self.x1, self.y1, self.nodata, self.nxdem, self.nydem,
                 self.alt_dem) = build_combined_raster(short_source, self.lat, self.lon, info_only)
            else:
                if ensure_elevation(short_source, self.lat, self.lon):
                    (self.epsg, self.x0, self.y0, self.x1, self.y1, self.nodata, self.nxdem, self.nydem, self.alt_dem) =\
                        read_elevation_from_file(FNAMES.elevation_data(
                            short_source, self.lat, self.lon), self.lat, self.lon, info_only, 3601)
                else:
                    (self.epsg, self.x0, self.y0, self.x1, self.y1, self.nodata, self.nxdem, self.nydem, self.alt_dem) =\
                        (4326, 0, 0, 1, 1, -32768, 3601, 3601,
                         numpy.zeros((3601, 3601), dtype=numpy.float32))
        else:
            file_name = source
            (self.epsg, self.x0, self.y0, self.x1, self.y1, self.nodata, self.nxdem, self.nydem,
             self.alt_dem) = read_elevation_from_file(file_name, self.lat, self.lon, info_only)
        if not local_sources:
            return
        self.subdems = tuple()
        for local_source in local_sources:
            self.subdems += (DEM(self.lat, self.lon,
                             local_source, False, info_only),)
    ...
    def create_normal_map(self, pixx, pixy):
        ...
        band_r = Image.fromarray(
            ((1+dx)/2*255).astype(numpy.uint8)).resize((4096, 4096))
        del(dx)
        band_g = Image.fromarray(
            ((1-dy)/2*255).astype(numpy.uint8)).resize((4096, 4096))
        del(dy)
    ...
    def alt_nostrict(self, node):
        ...
        t2 = self.alt_dem[(Nminusny-1)*(Nminusny >= 1),
                          (nx+1)*(nx < Nx)+Nx*(nx == Nx)]
    ...
    def alt_vec_nostrict(self, way):
        ...
        t2 = [self.alt_dem[i][j] for i, j in zip(
    ...
    def alt_vec_strict(self, way):
        ...
        nx = numpy.round((x-self.x0)/(self.x1-self.x0) *
                         (self.nxdem-1)).astype(numpy.uint16)
        Nminusny = numpy.round((self.y1-y)/(self.y1-self.y0)
                               * (self.nydem-1)).astype(numpy.uint16)
...
def build_combined_raster(source, lat, lon, info_only):
    world_tiles = numpy.array(Image.open(
        os.path.join(FNAMES.Utils_dir, 'world_tiles.png')))
    ...
    for (lat0, lon0) in itertools.product((lat, lat-1, lat+1), (lon, lon-1, lon+1)):
        ...
        if not world_tiles[y, x]:
            ...
            tmparray = read_elevation_from_file(FNAMES.elevation_data(
                source, lat0, (lon0+180) % 360-180), lat0, (lon0+180) % 360-180, info_only, base)[-1]
        else:
            tmparray = numpy.zeros((base, base), dtype=numpy.float32)
        by = beyond
        ov = overlap
        if lat0 == lat and lon0 == lon:
            ...
            alt_dem[by:-by, :by] = tmparray[:, -by -
                                            ov:-ov] if ov else tmparray[:, -by:]
        elif lat0 == lat and lon0 == lon+1:
            alt_dem[by:-by, -by:] = tmparray[:,
                                             ov:ov+by] if ov else tmparray[:, :by]
        elif lat0 == lat+1 and lon0 == lon:
            alt_dem[:by, by:-by] = tmparray[-ov-by:-
                                            ov, :] if ov else tmparray[-by:, :]
        elif lat0 == lat-1 and lon0 == lon:
            alt_dem[-by:, by:-by] = tmparray[ov:ov +
                                             by, :] if ov else tmparray[:by, :]
        elif lat0 == lat+1 and lon0 == lon-1:
            alt_dem[:by, :by] = tmparray[-ov-by:-ov, -
                                         ov-by:-ov] if ov else tmparray[-by:, -by:]
        elif lat0 == lat+1 and lon0 == lon+1:
            alt_dem[:by, -by:] = tmparray[-ov-by:-ov,
                                          ov:ov+by] if ov else tmparray[-by:, :by]
        elif lat0 == lat-1 and lon0 == lon-1:
            alt_dem[-by:, :by] = tmparray[ov:ov+by, -
                                          ov-by:-ov] if ov else tmparray[:by, -by:]
        elif lat0 == lat-1 and lon0 == lon+1:
            alt_dem[-by:, -by:] = tmparray[ov:ov+by,
                                           ov:ov+by] if ov else tmparray[:by, :by]
...
def read_elevation_from_file(file_name, lat, lon, info_only=False, base_if_error=3601):
    ...
    if file_name[-4:].lower() == '.hgt':
        ...
        try:
            ...
            if not info_only:
                alt_dem = numpy.fromfile(file_name, numpy.dtype('>i2')).astype(
                    numpy.float32).reshape((nydem, nxdem))
            if nxdem == 1201:
                nxdem = nydem = 3601
                if not info_only:
            ...
            UI.lvprint(1, "    ERROR: in reading elevation from",
                       file_name, "-> replaced with zero altitude.")
        ...
        try:
            ...
            if nodata is None:
                UI.vprint(
                    1, "    WARNING: raster DEM does not advertise its no_data value, assuming -32768.")
                nodata = -32768
            ...
            try:
                ...
                UI.vprint(
                    1, "    WARNING: raster DEM does not advertise its EPSG code, assuming 4326.")
                epsg = 4326
            # let's be blind about 4269 which might be sufficiently close to 4326 for our purposes
            if epsg not in (4326, 4269):
                UI.lvprint(1, "    WARNING: unsupported EPSG code ", epsg,
            ...
            UI.lvprint(1, "   ERROR: in reading ", file_name,
        ...
        UI.lvprint(1, "   WARNING: unsupported raster (install Gdal):",
                   file_name, "-> replaced with zero altitude.")
        nxdem = nydem = base_if_error
        if not info_only:
            alt_dem = numpy.zeros(
...
def ensure_elevation(source, lat, lon, verbose=True):
    if source == 'View':
        ...
        if (lat, lon) in ((44, 5), (45, 5), (46, 5), (43, 6), (44, 6), (45, 6), (46, 6), (47, 6), (43, 7), (44, 7), (45, 7),
            ...
            deferranti_letter = alphabet[lat //
                                         4] if lat >= 0 else alphabet[(-1-lat)//4]
            if lat < 0:
            ...
            if deferranti_letter+deferranti_nbr in (
               ...
               "O34", "P34", "Q34", "R34", "O35", "P35", "Q35", "R35", "P36", "Q36", "R36"):
                ...
                resol = 3
            url = "http://viewfinderpanoramas.org/dem" + \
                str(resol)+"/"+deferranti_letter+deferranti_nbr+".zip"
        if os.path.exists(FNAMES.viewfinderpanorama(lat, lon)) and (resol == 3 or os.path.getsize(FNAMES.viewfinderpanorama(lat, lon)) >= 25934402):
            ...
            return 1
        UI.vprint(1, "    Downloading ", FNAMES.viewfinderpanorama(
            lat, lon), "from Viewfinderpanoramas (J. de Ferranti).")
        ...
        with zipfile.ZipFile(io.BytesIO(r.content), "r") as zip_ref:
            for f in zip_ref.filelist:
                ...
                try:
                    ...
                    UI.vprint(
                        2, "      Archive contains the unknown file name", fname, "which is skipped.")
                    continue
        ...
        if os.path.exists(FNAMES.elevation_data(source, lat, lon)):
            UI.vprint(2, "   Recycling ",
                      FNAMES.elevation_data(source, lat, lon))
            return 1
        UI.vprint(1, "    Downloading ", FNAMES.elevation_data(
            source, lat, lon), "from OpenTopography (SDSC).")
        ...
        if not r:
            ...
            r = http_request(url_base+'n'+str(lat + 1).zfill(2) +
                             'w'+str(-lon).zfill(3)+'.zip', source, verbose)
            if not r:
                return 0
        with zipfile.ZipFile(io.BytesIO(r.content), "r") as zip_ref:
            if not os.path.isdir(os.path.dirname(FNAMES.elevation_data(source, lat, lon))):
        ...
        r = http_request(url_base+'USGS_NED_1_'+usgs_base +
                         '_ArcGrid.zip', source, verbose)
        if not r:
            UI.vprint(2, "    Trying alternative naming scheme.")
...
def http_request(url, source, verbose=False):
    ...
    while True:
        ...
        if tentative == 6:
            return 0
        UI.vprint(1, "    ", source,
                  "server may be down or busy, new tentative in", 2**tentative, "sec...")
    ...
    while (alt_dem == nodata).any():
        if not step:
            if numpy.sum(alt_dem == nodata) >= 10000:
                return 0
            UI.vprint(
                2, "    INFO: Elevation file contains voids, trying to fill them recursively by nearest neighbour.")
        else:
            UI.vprint(2, "    ", step)
        ...
        if step > 20:
            UI.vprint(
                1, "    WARNING: The raster contain holes that seem to big to be filled... I'm filling the remainder with zero.")
...
def upsample(alt_dem):
    ...
    for i in range(1201):
        ...
        alt_dem_tmp[3*i+1, 1::3] = 4/9*alt_dem[i][:-1]+2/9 * \
            alt_dem[i, 1:]+2/9*alt_dem[i+1, :-1]+1/9*alt_dem[i+1, 1:]
        alt_dem_tmp[3*i+2, 1::3] = 2/9*alt_dem[i][:-1]+1/9 * \
            alt_dem[i, 1:]+4/9*alt_dem[i+1, :-1]+2/9*alt_dem[i+1, 1:]
        alt_dem_tmp[3*i+1, 2::3] = 2/9*alt_dem[i][:-1]+4/9 * \
            alt_dem[i, 1:]+1/9*alt_dem[i+1, :-1]+2/9*alt_dem[i+1, 1:]
        alt_dem_tmp[3*i+2, 2::3] = 1/9*alt_dem[i][:-1]+2/9 * \
            alt_dem[i, 1:]+2/9*alt_dem[i+1, :-1]+4/9*alt_dem[i+1, 1:]
    return alt_dem_tmp
##############################################################################
...
def smoothen(raster, pix_width, mask_im, preserve_boundary=True):
    ...
    tmp = tmp.transpose()
    tmpw = tmpw.transpose()
    tmp[mask_array != 0] = mask_array[mask_array != 0]*tmp[mask_array != 0] / \
        tmpw[mask_array != 0] + \
    ...
    if preserve_boundary:
        for i in range(pix_width):
            ...
            tmp[-i-1] = i/pix_width*tmp[-i-1] + \
                (pix_width-i)/pix_width*raster[-i-1]
        for i in range(pix_width):
            tmp[:, i] = i/pix_width*tmp[:, i] + \
