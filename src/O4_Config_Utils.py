...
list_vector_vars = ['apt_smoothing_pix', 'road_level', 'road_banking_limit', 'lane_width',
                    'max_levelled_segs', 'water_simplification', 'min_area', 'max_area', 'clean_bad_geometries', 'mesh_zl']
list_mesh_vars = ['curvature_tol', 'apt_curv_tol', 'apt_curv_ext', 'coast_curv_tol', 'coast_curv_ext',
                  'limit_tris', 'hmin', 'min_angle', 'sea_smoothing_mode', 'water_smoothing', 'iterate']
list_mask_vars = ['mask_zl', 'masks_width', 'masking_mode', 'use_masks_for_inland',
                  'imprint_masks_to_dds', 'masks_use_DEM_too', 'masks_custom_extent']
list_dsf_vars = ['cover_airports_with_highres', 'cover_extent', 'cover_zl', 'ratio_water', 'overlay_lod', 'sea_texture_blur',
                 'add_low_res_sea_ovl', 'experimental_water', 'normal_map_strength', 'terrain_casts_shadows', 'use_decal_on_terrain']
list_other_vars = ['custom_dem', 'fill_nodata']
list_tile_vars = list_vector_vars+list_mesh_vars+list_mask_vars + \
    list_dsf_vars+list_other_vars + \
    ['default_website', 'default_zl', 'zone_list']

list_global_cfg = list_app_vars+list_vector_vars + \
    list_mesh_vars+list_mask_vars+list_dsf_vars

############################################################################################
# Initialization to default values
for var in cfg_vars:
    target = cfg_vars[var]['module']+"." + \
        var if 'module' in cfg_vars[var] else var
...
class Tile():
    ...
    def __init__(self, lat, lon, custom_build_dir):
        ...
        self.grouped = True if (
            custom_build_dir and custom_build_dir[-1] != '/') else False
    ...
    def make_dirs(self):
        if os.path.isdir(self.build_dir):
            if not os.access(self.build_dir, os.W_OK):
                UI.vprint(0, "OS error: Tile directory",
                          self.build_dir, " is write protected.")
                raise Exception
        else:
            try:
                os.makedirs(self.build_dir)
            except:
                UI.vprint(0, "OS error: Cannot create tile directory",
                          self.build_dir, " check file permissions.")
                raise Exception
    ...
    def read_from_config(self, config_file=None):
        if not config_file:
            ...
            if not os.path.isfile(config_file):
                ...
                if not os.path.isfile(config_file):
                    UI.lvprint(0, "CFG error: No config file found for tile",
        ...
        try:
            ...
            UI.lvprint(0, "CFG error: Could not read config file for tile",
                       FNAMES.short_latlon(self.lat, self.lon))
            return 0
    ...
    def write_to_config(self, config_file=None):
        ...
        try:
            ...
            UI.vprint(2, e)
            UI.lvprint(0, "CFG error: Could not write config file for tile",
                       FNAMES.short_latlon(self.lat, self.lon))
            return 0
...
class Ortho4XP_Config(tk.Toplevel):
    ...
    def choose_dem(self):
        tmp = filedialog.askopenfilename(parent=self, title='Choose DEM file', filetypes=[
    ...
    def load_tile_cfg(self):
        ...
        try:
            f = open(os.path.join(build_dir, 'Ortho4XP_' +
                     FNAMES.short_latlon(lat, lon)+'.cfg'), 'r')
        except:
            try:
    ...
    def write_tile_cfg(self):
        ...
        try:
            ...
            f = open(os.path.join(build_dir, 'Ortho4XP_' +
                     FNAMES.short_latlon(lat, lon)+'.cfg'), 'w')
        except:
    ...
    def apply_changes(self):
        ...
        for var in list_tile_vars+list_app_vars:
            try:
                target = cfg_vars[var]['module']+"." + \
                    var if 'module' in cfg_vars[var] else "globals()['" + \
                    var+"']"
                if cfg_vars[var]['type'] in (bool, list):
                    cmd = target+"="+self.v_[var].get()
                else:
                    cmd = target+"=cfg_vars['"+var + \
                        "']['type'](self.v_['"+var+"'].get())"
                exec(cmd)
            except:
                target = cfg_vars[var]['module']+"." + \
                    var if 'module' in cfg_vars[var] else "globals()['" + \
                ...
                exec(
                    target+"=cfg_vars['"+var+"']['type'](cfg_vars['"+var+"']['default'])")
                errors.append(var)
        if errors:
