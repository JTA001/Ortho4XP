...
def initialize_extents_dict():
    for dir_name in os.listdir(FNAMES.Extent_dir):
        ...
        for file_name in os.listdir(os.path.join(FNAMES.Extent_dir, dir_name)):
            ...
            extent = {}
            f = open(os.path.join(FNAMES.Extent_dir, dir_name,
...
def initialize_color_filters_dict():
    for file_name in os.listdir(FNAMES.Filter_dir):
        ...
        if valid_color_filters:
            ...
            print("Could not understand color filter ",
                  color_code, ", skipping it.")
            pass
        f.close()
        ...
        for file_name in os.listdir(os.path.join(FNAMES.Provider_dir, dir_name)):
            ...
            for line in f.readlines():
                ...
                if key == 'request_type' and value not in ['wms', 'wmts', 'tms', 'local_tms']:
                    UI.vprint(0, "Unknown request_type field for provider",
                              provider_code, ":", value)
                    valid_provider = False
                if key == 'grid_type' and value not in ['webmercator']:
                    UI.vprint(0, "Unknown grid_type field for provider",
                              provider_code, ":", value)
                    valid_provider = False
                    ...
                    try:
                        ...
                        if type(provider[key]) is not dict:
                            print("Definition of fake headers for provider",
                    ...
                    try:
                        ...
                        if provider[key] < 100 or provider[key] > 10000:
                            print("Wm(t)s size for provider ", provider_code,
                                  "seems off limits, provider skipped.")
                    except:
                        print("Error in reading wms size for provider",
                              provider_code)
                        valid_provider = False
                elif key in ['wms_version', 'wmts_version']:
                    if len(value.split('.')) < 2:
                        print("Error in reading wms version for provider",
                              provider_code)
                        valid_provider = False
                elif key == 'top_left_corner':
                    try:
                        provider[key] = [numpy.array(
                            [float(x) for x in value.split()]) for _ in range(40)]
                    except:
                        print(
                            "Error in reading top left corner for provider", provider_code)
                        valid_provider = False
                    ...
                    try:
                        ...
                        print(
                            "Error in reading scaledenominator for provider", provider_code)
                        valid_provider = False
                    ...
                    try:
                        ...
                        print("Error in reading resolutions for provider",
                              provider_code)
                        valid_provider = False
                    ...
                    if value not in color_filters_dict:
                        print("Error in reading color_filter for provider",
                              provider_code, ". Assuming none.")
                        provider[key] = 'none'
                elif key == 'imagery_dir':
                    if value not in ('grouped', 'normal', 'code'):
                        print("Error in reading imagery_dir for provider",
                              provider_code, ". Assuming grouped.")
                        provider[key] = 'grouped'
            if 'request_type' in provider and provider['request_type'] == 'wmts':
                try:
                    tilematrixsets = read_tilematrixsets(os.path.join(
                        FNAMES.Provider_dir, dir_name, 'capabilities_'+provider_code+'.xml'))
                except:
                    try:
                        tilematrixsets = read_tilematrixsets(os.path.join(
                    ...
                    try:
                        ...
                        if tms_found:
                            provider['scaledenominator'] = numpy.array(
                                [float(x['ScaleDenominator']) for x in provider['tilematrixset']['tilematrices']])
                            provider['top_left_corner'] = [[float(x) for x in y['TopLeftCorner'].split(
            ...
            if valid_provider:
                ...
                if 'scaledenominator' in provider:
                    units_per_pix = 0.00028 if provider['epsg_code'] not in [
                        '4326'] else 2.5152827955e-09
                    provider['resolutions'] = units_per_pix * \
                        provider['scaledenominator']
        ...
        try:
            ...
            for line in f.readlines():
                ...
                if layer_code not in providers_dict:
                    print("Unknown provider in combined provider",
                ...
                if (extent_code not in extents_dict) or (extent_code[0] == '!' and extent_code[1:] not in extents_dict):
                    print("Unknown extent in combined provider",
                ...
                if color_code == 'default':
                    try:
                        ...
                        print("Unknown color filter in combined provider",
                ...
                if priority not in ['low', 'medium', 'high', 'mask']:
                    print("Unknown priority in combined provider",
                          provider_code, ":", priority,)
                    continue
                comb_list.append({'layer_code': layer_code, 'extent_code': extent_code,
                                 'color_code': color_code, 'priority': priority})
            f.close()
            if comb_list:
                combined_providers_dict[provider_code] = comb_list
            else:
                print("Combined provider", provider_code,
...
def initialize_local_combined_providers_dict(tile):
    ...
    for provider_code in test_set.intersection(combined_providers_dict):
        ...
        if comb_list:
            if len(comb_list) != 1:
                ...
                for rlayer in comb_list:
                    ...
                    if extents_dict[name]['dir'] == 'LowRes':
                        ...
                        new_extent_code = name+"_" + \
                            FNAMES.short_latlon(tile.lat, tile.lon)
                        new_rlayer['extent_code'] = new_extent_code
                        new_comb_list.append(new_rlayer)
                        extents_dict[new_extent_code] = {'dir': 'Auto', 'code': new_extent_code, 'mask_bounds': [
                            tile.lon-0.1, tile.lat-0.1, tile.lon+1.1, tile.lat+1.1]}
                        ...
                        if not os.path.exists(cached_file_name):
                            UI.vprint(
                                0, "Error, missing OSM data for extent code", name, ", exiting.")
                        ...
                        if not multipolygon_area.area:
                            UI.vprint(
                                0, "Error, erroneous OSM data for extent code", name, ", skipped.")
                            continue
                        vector_map.encode_MultiPolygon(
                            multipolygon_area, VECT.dummy_alt, 'WATER', check=False, cut=False)
                        vector_map.write_node_file(name+'.node')
                        vector_map.write_poly_file(name+'.poly')
                        MESH.triangulate(name, '.')
                        ((xmin, ymin, xmax, ymax), mask_im) = MASK.triangulation_to_image(
                            name, pixel_size, (tile.lon-0.1, tile.lat-0.1, tile.lon+1.1, tile.lat+1.1))
                        if buffer_width:
                            ...
                            if buffer_width > 0:
                                ...
                                mask_im = Image.fromarray(
                                    (numpy.array(mask_im, dtype=numpy.uint8) == 255).astype(numpy.uint8)*255)
                        if mask_width:
                            mask_width += 1
            ...
            UI.vprint(1, "Combined provider", provider_code,
...
def has_data(bbox, extent_code, return_mask=False, mask_size=(4096, 4096), is_sharp_resize=False, is_mask_layer=False):
    ...
    try:
        ...
        if extent_code[0] == '!':
            ...
            negative = False
        (xmin, ymin, xmax,
         ymax) = extents_dict[extent_code]['mask_bounds'] if extent_code != 'global' else (-180, -90, 180, 90)
        if x0 > xmax or x1 < xmin or y0 < ymin or y1 > ymax:
            return negative
        if (not is_mask_layer) or (x1-x0) == 1:
            mask_im = Image.open(os.path.join(
                FNAMES.Extent_dir, extents_dict[extent_code]['dir'], extents_dict[extent_code]['code']+".png")).convert("L")
            ...
            mask_im2 = Image.open(os.path.join(
                check_dir, FNAMES.legacy_mask(m_tilx, m_tily))).convert("L")
...
def get_wmts_image(tilematrix, til_x, til_y, provider, http_session):
    ...
    while True:
        ...
        if has_URL and provider['code'] in URL.custom_url_list:
            ...
            url = url.replace('{quadkey}', GEO.gtile_to_quadkey(
                til_x, til_y, tilematrix))
            url = url.replace('{xcenter}', str(
                (til_x+0.5)*provider['resolutions'][tilematrix]*provider['tile_size']+provider['top_left_corner'][tilematrix][0]))
            url = url.replace('{ycenter}', str(-1*(til_y+0.5) *
                              provider['resolutions'][tilematrix]*provider['tile_size']+provider['top_left_corner'][tilematrix][1]))
            url = url.replace('{size}', str(
                int(provider['resolutions'][tilematrix]*provider['tile_size'])))
            if '{switch:' in url:
            ...
            url = provider['url_prefix']+"&SERVICE=WMTS&VERSION=1.0.0&REQUEST=GetTile&LAYER=" +\
                ...
                "&TILEMATRIX="+provider['tilematrixset']['tilematrices'][tilematrix]['identifier'] + \
...
def build_texture_from_tilbox(tilbox, zoomlevel, provider, progress=None):
    ...
    for monty in range(0, parts_y):
        for montx in range(0, parts_x):
            x0 = montx*width
            y0 = monty*height
            fargs = (zoomlevel, til_x_min+montx, til_y_min+monty,
...
def build_texture_from_bbox_and_size(t_bbox, t_epsg, t_size, provider):
    ...
    if provider['request_type'] == 'wms':
        ...
        if s_box_ulx != s_ulx or s_box_uly != s_uly or s_box_lrx != s_lrx or s_box_lry != s_lry:
            ...
            s_lry = s_box_lry
            crop_needed = True
        downscale = int(min(log(width*parts_x/t_sizex),
                        log(height/t_sizey))/log(2))-1
        if downscale >= 1:
            width /= 2**downscale
            height /= 2**downscale
    ...
    for monty in range(0, parts_y):
        for montx in range(0, parts_x):
            ...
            if provider['request_type'] == 'wms':
                ...
                fargs = [p_bbox[:], width, height, provider,
                         big_image, x0, y0, http_session]
            elif provider['request_type'] in ['wmts', 'tms', 'local_tms']:
                fargs = [wmts_tilematrix, til_x_min+montx, til_y_min+monty,
    ...
    if warp_needed:
        ...
        big_image = gdalwarp_alternative(
            (s_ulx, s_uly, s_lrx, s_lry), provider['epsg_code'], big_image, t_bbox, t_epsg, t_size)
    elif crop_needed:
        UI.vprint(3, "Crop needed")
        big_image = big_image.crop((crop_x0, crop_y0, crop_x1, crop_y1))
    if big_image.size != t_size:
        UI.vprint(3, "Resize needed:" +
                  str(t_size[0]/big_image.size[0])+" "+str(t_size[1]/big_image.size[1]))
...
def download_jpeg_ortho(file_dir, file_name, til_x_left, til_y_top, zoomlevel, provider_code, super_resol_factor=1):
    ...
    if UI.red_flag:
        return 0
    if not success:
        UI.lvprint(1, "Part of image", file_name,
    ...
    try:
        if super_resol_factor == 1:
            ...
            big_image.resize((int(width/super_resol_factor), int(height/super_resol_factor)),
                             Image.BICUBIC).save(os.path.join(file_dir, file_name))
    except Exception as e:
        UI.lvprint(
...
def build_jpeg_ortho(tile, til_x_left, til_y_top, zoomlevel, provider_code, out_file_name=''):
    ...
    if provider_code in local_combined_providers_dict:
        ...
        for rlayer in local_combined_providers_dict[provider_code]:
            ...
            if len(local_combined_providers_dict[provider_code]) == 1 or has_data((x0, y0, x1, y1), rlayer['extent_code'], is_mask_layer=(tile.lat, tile.lon, tile.mask_zl) if rlayer['priority'] == 'mask' else False):
                ...
                if 'max_zl' in providers_dict[rlayer['layer_code']]:
                    max_zl = int(
                        providers_dict[rlayer['layer_code']]['max_zl'])
                    if max_zl < zoomlevel:
                ...
                if not os.path.isfile(os.path.join(true_file_dir, true_file_name)):
                    UI.vprint(1, "   Downloading missing orthophoto " +
                    ...
                    UI.vprint(1, "   The orthophoto "+true_file_name +
        ...
        UI.vprint(1, "   Unknown provider", provider_code,
...
def build_combined_ortho(tile, latp, lonp, zoomlevel, provider_code, mask_zl, filename='test.png'):
    ...
    for rlayer in combined_providers_dict[provider_code][::-1]:
        mask = has_data((x0, y0, x1, y1), rlayer['extent_code'], return_mask=True, is_mask_layer=(
            tile.lat, tile.lon, tile.mask_zl) if rlayer['priority'] == 'mask' else False)
        if not mask:
            continue
        ...
        if not os.path.isfile(os.path.join(true_file_dir, true_file_name)):
            UI.vprint(1, "   Downloading missing orthophoto " +
                      true_file_name+" (for combining in "+provider_code+")\n")
            download_jpeg_ortho(true_file_dir, true_file_name, true_til_x_left,
                                true_til_y_top, true_zl, rlayer['layer_code'])
        else:
            UI.vprint(1, "   The orthophoto "+true_file_name +
                      " (for combining in "+provider_code+") is already present.\n")
        ...
        if rlayer['priority'] == 'mask' and tile.sea_texture_blur:
            ...
            true_im = true_im.filter(ImageFilter.GaussianBlur(
        ...
        if rlayer['priority'] == 'low':
            ...
            mask[wasnt_zero] = 255*mask[wasnt_zero] / \
                (mask_weight_below+mask)[wasnt_zero]
        ...
        for til_x_left in range(til_xmin, til_xmax+1, 16):
            ...
            if has_data(bbox_4326, providers_dict[provider_code]['extent'], return_mask=False, mask_size=(4096, 4096)):
                ...
                if os.path.isfile(os.path.join(dest_dir, file_name)):
                    ...
                    continue
                print("building one")
                download_jpeg_ortho(dest_dir, file_name, til_x_left, til_y_top,
...
def build_provider_texture(dest_dir, provider_code, zoomlevel):
    (lonmin, latmin, lonmax,
     latmax) = extents_dict[providers_dict[provider_code]['extent']]['mask_bounds']
    build_texture_region(dest_dir, latmin, latmax, lonmin,
                         lonmax, zoomlevel, provider_code)
    return
###############################################################################################################################
    ...
    try:
        for color_filter in color_filters_dict[color_code]:
            ...
            if color_filter[0] == 'brightness-contrast':
                ...
                if brightness >= 0:
                    im = im.point(lambda i: 128+tan(pi/4*(1+contrast/128))
                                  * (brightness+(255-brightness)/255*i-128))
                else:
                    im = im.point(
                        lambda i: 128+tan(pi/4*(1+contrast/128))*((255+brightness)/255*i-128))
            ...
            # levels range between 0 and 255, gamma is neutral at 1 / https://pippin.gimp.org/image-processing/chap_point.html
                ...
                for j in [0, 1, 2]:
                    ...
                    bands[j].paste(bands[j].point(lambda i: out_min+(out_max-out_min)*(
                        (max(in_min, min(i, in_max))-in_min)/(in_max-in_min))**(1/gamma)))
...
def combine_textures(tile, til_x_left, til_y_top, zoomlevel, provider_code):
    ...
    for rlayer in local_combined_providers_dict[provider_code][::-1]:
        ...
        true_arr = numpy.array(true_im).astype(numpy.uint16)
        mask[(numpy.sum(true_arr, axis=2) >= 735)
             * (mask >= 1)*(mask <= 253)] = 0
...
def convert_texture(tile, til_x_left, til_y_top, zoomlevel, provider_code, type='dds'):
    ...
    if tile.imprint_masks_to_dds and type == 'dds':
        masked_texture = os.path.exists(os.path.join(tile.build_dir, "textures", FNAMES.mask_file(
            til_x_left, til_y_top, zoomlevel, provider_code)))
        if masked_texture:
            mask_im = Image.open(os.path.join(tile.build_dir, "textures", FNAMES.mask_file(
                til_x_left, til_y_top, zoomlevel, provider_code))).convert('L')
        ...
        if int(zoomlevel) >= tile.mask_zl:
            ...
            mask_file = os.path.join(FNAMES.mask_dir(
                tile.lat, tile.lon), FNAMES.legacy_mask(m_til_x, m_til_y))
    ...
    if (provider_code in local_combined_providers_dict) and ((provider_code not in providers_dict) or not os.path.exists(os.path.join(file_dir, jpeg_file_name))):
        ...
        if masked_texture:
            ...
            if type == 'dds':
                try:
                    os.remove(os.path.join(tile.build_dir, "textures", FNAMES.mask_file(
                        til_x_left, til_y_top, zoomlevel, provider_code)))
                except:
                    pass
            dxt5 = True
    ...
    # now if provider_code was not in local_combined_providers_dict but color correction is required
        ...
        big_image = Image.open(os.path.join(
            file_dir, jpeg_file_name), 'r').convert('RGB')
    ...
    if type == 'dds':
        ...
        if latmax-latmin < 0.04:
            conv_cmd = [gdal_transl_cmd, '-of', 'Gtiff', '-co', 'COMPRESS=JPEG', '-a_ullr', str(lonmin), str(latmax), str(
                lonmax), str(latmin), '-a_srs', 'epsg:4326', file_to_convert, os.path.join(FNAMES.Geotiff_dir, out_file_name)]
        else:
            geotag_cmd = [gdal_transl_cmd, '-of', 'Gtiff', '-co', 'COMPRESS=JPEG', '-a_ullr', str(xmin), str(
                ymax), str(xmax), str(ymin), '-a_srs', 'epsg:3857', file_to_convert, tmp_tif_file_name]
            erase_tmp_tif = True
            if subprocess.call(geotag_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT):
                UI.vprint(1, "ERROR: Could not geotag texture (gdal not present ?) ", os.path.join(
                    tile.build_dir, 'textures', out_file_name))
                try:
                    os.remove(os.path.join(
                        UI.Ortho4XP_dir, 'tmp', png_file_name))
                except:
                    pass
                return
            conv_cmd = [gdalwarp_cmd, '-of', 'Gtiff', '-co', 'COMPRESS=JPEG', '-s_srs', 'epsg:3857', '-t_srs', 'epsg:4326',
                        '-ts', '4096', '4096', '-rb', tmp_tif_file_name, os.path.join(FNAMES.Geotiff_dir, out_file_name)]
    tentative = 0
    while True:
        ...
        tentative += 1
        if tentative == 10:
            UI.lvprint(1, "ERROR: Could not convert texture", os.path.join(
                tile.build_dir, 'textures', out_file_name), "(10 tries)")
            break
        UI.lvprint(1, "WARNING: Could not convert texture",
...
def geotag(input_file_name):
    ...
    (latmin, lonmax) = GEO.gtile_to_wgs84(
        til_x_left+16, til_y_top+16, zoomlevel)
    conv_cmd = [gdal_transl_cmd, '-of', 'Gtiff', '-co', 'COMPRESS=JPEG', '-a_ullr', str(lonmin), str(
        latmax), str(lonmax), str(latmin), '-a_srs', 'epsg:4326', input_file_name, out_file_name]
    tentative = 0
    while True:
        ...
        tentative += 1
        if tentative == 10:
            print("ERROR: Could not convert texture",
                  out_file_name, "(10 tries)")
            break
