...
def community_mesh(tile):
    if not community_server:
        ...
        return 0
    url = community_prefix + \
        os.path.basename(FNAMES.mesh_file(
            tile.build_dir, tile.lat, tile.lon))+'.7z'
    timer = time.time()
    ...
    try:
        ...
        if '[200]' in str(r):
            ...
            f = open(FNAMES.mesh_file(tile.build_dir,
                     tile.lat, tile.lon)+'.7z', 'wb')
            f.write(r.content)
            f.close()
    ...
    if tile.coast_curv_tol != tile.curvature_tol:
        ...
        for nodeid in sea_layer.dicosmn:
            ...
            weight_array[rowmin:rowmax+1, colmin:colmax+1] = numpy.maximum(
                weight_array[rowmin:rowmax+1, colmin:colmax+1], tile.curvature_tol/tile.coast_curv_tol)
        del(sea_layer)
...
def post_process_nodes_altitudes(tile):
    ...
    for i in range(0, nbr_pt):
        vertices[6*i:6*i+6] = [float(x)
                               for x in f_node.readline().split()[1:7]]
    ...
    for i in range(0, nbr_pt):
        f_node.write(
            str(i+1)+" "+' '.join(('{:.15f}'.format(x) for x in vertices[6*i:6*i+6]))+"\n")
...
def write_mesh_file(tile, vertices):
    UI.vprint(1, "-> Writing final mesh to the file " +
...
def extract_mesh_to_obj(mesh_file, til_x_left, til_y_top, zoomlevel, provider_code):
    ...
    for i in range(1, nbr_vert+1):
        j = textured_nodes[i]
        f.write("vn "+'{:.9f}'.format(pt_in[5*j+3])+" "+'{:.9f}'.format(
            pt_in[5*j+4])+" "+'{:.9f}'.format(sqrt(max(1-pt_in[5*j+3]**2-pt_in[5*j+4]**2, 0)))+"\n")
    f.write("\n")
    for i in range(1, nbr_vert+1):
        j = textured_nodes[i]
    ...
    for i in range(0, nbr_tri):
        ...
        f.write("f "+str(one)+"/"+str(one)+"/"+str(one)+" "+str(two)+"/" +
                str(two)+"/"+str(two)+" "+str(three)+"/"+str(three)+"/"+str(three)+"\n")
    f_mesh.close()
    f.close()
    # then the mtl file
    f = open(mtl_file_name, 'w')
    f.write("newmtl orthophoto\nmap_Kd "+FNAMES.geotiff_file_name_from_attributes(
        til_x_left, til_y_top, zoomlevel, provider_code)+"\n")
    f.close()
    UI.timings_and_bottom_line(timer)
    return
...
def build_mesh(tile):
    ...
    UI.logprint("Step 2 for tile lat=", tile.lat,
    ...
    if not tile.iterate:
        ...
        try:
            source = ((";" in tile.custom_dem) and tile.custom_dem.split(
                ";")[tile.iterate]) or tile.custom_dem
            tile.dem = DEM.DEM(tile.lat, tile.lon, source,
                               fill_nodata=False, info_only=True)
            if not os.path.isfile(alt_file) or not os.path.getsize(alt_file) == 4*tile.dem.nxdem*tile.dem.nydem:
                tile.dem = DEM.DEM(tile.lat, tile.lon, source,
                                   fill_nodata=False, info_only=False)
    ...
    limit_tris = 'S' + \
        str(max(int(tile.limit_tris/1.9-input_nodes), 0)) if tile.limit_tris else ''
...
def sort_mesh(tile):
    ...
    if not os.path.isfile(mesh_file):
        ...
        return 0
    sort_mesh_cmd_list = [
        sort_mesh_cmd.strip(), str(tile.default_zl), mesh_file]
    ...
    while True:
        ...
        if not line:
            ...
            print(line.decode("utf-8")[:-1])
    UI.timings_and_bottom_line(timer)
    UI.logprint("Moulinette applied for tile lat=", tile.lat,
                ", lon=", tile.lon, " and ZL", tile.default_zl)
    return 1
##############################################################################
...
def triangulate(name, path_to_Ortho4XP_dir):
    Tri_option = ' -pAYPQ '
    mesh_cmd = [os.path.join(path_to_Ortho4XP_dir, triangle_cmd).strip(
    ), Tri_option.strip(), name+'.poly']
    fingers_crossed = subprocess.Popen(
        mesh_cmd, stdout=subprocess.PIPE, bufsize=0)
    while True:
        line = fingers_crossed.stdout.readline()
