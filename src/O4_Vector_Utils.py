...
class Vector_Map():
    ...
    dico_attributes = {'DUMMY': 0, 'WATER': 1, 'SEA': 2, 'SEA_EQUIV': 4,
    ...
    def insert_edge(self, id0, id1, marker, check=True):
        ...
        for hits in task:
            ...
            if len(coeffs) == 2:  # transverse encroachment
                ...
                if beta not in (0, 1):
                    c_x = (1-alpha) * \
                        self.nodes_dico[id0][0]+alpha*self.nodes_dico[id1][0]
                    c_y = (1-alpha) * \
                        self.nodes_dico[id0][1]+alpha*self.nodes_dico[id1][1]
                    # ! important to rely on the old id2 id3 for the z value !
                    c_z = (1-beta) * \
                        self.data_nodes[id2]+beta*self.data_nodes[id3]
    ...
    def are_encroached(self, a, b, c, d):
        ...
        if abs(numpy.linalg.det(A)) > eps:
            ...
            alpha0, alpha1 = (a-c)[g_idx] / \
                (a-b)[g_idx], (a-d)[g_idx]/(a-b)[g_idx]
    ...
    def snap_to_grid(self, digits):
        ...
        for (id0, id1) in self.dico_edges:
            ...
            if (id0n, id1n) in dico_edges_new:
                ...
                data_edges_new[eid] = data_edges_new[eid] | self.data_edges[self.dico_edges[(
                    id0, id1)]]  # bitwise add new marker if necessary
                ...
                data_edges_new[next_edge_id] = self.data_edges[self.dico_edges[(
                    id0, id1)]]
                next_edge_id += 1
        UI.vprint(2, "Simplified ", len(self.dico_nodes)-len(dico_nodes_new),
                  "duplicate nodes and", len(self.dico_edges)-len(dico_edges_new), "zero length edges.")
        (self.dico_nodes, self.nodes_dico, self.dico_edges, self.edges_dico, self.data_nodes, self.data_edges) = (
            dico_nodes_new, nodes_dico_new, dico_edges_new, edges_dico_new, data_nodes_new, data_edges_new)
    ...
    def write_node_file(self, node_file_name):
        ...
        for idx in sorted(self.nodes_dico.keys()):
            f.write(str(idx)+' '+' '.join(['{:.15f}'.format(x) for x in (
                self.nodes_dico[idx][0], self.nodes_dico[idx][1], self.data_nodes[idx])])+'\n')
        f.close()
    ...
    def write_poly_file(self, poly_file_name):
        ...
        for edge_id in self.edges_dico:
            f.write(str(idx)+' '+str(self.edges_dico[edge_id][0])+' '+str(
                self.edges_dico[edge_id][1])+' '+str(self.data_edges[edge_id])+'\n')
            idx += 1
        f.write('\n'+str(len(self.holes))+'\n')
        idx = 1
        ...
        if total_seeds == 0:
            ...
            for long_key in sorted(self.dico_attributes.items(), key=lambda item: item[1]):
                ...
                for seed in self.seeds[key]:
                    f.write(
                        str(idx)+' '+' '.join(['{:.15f}'.format(s) for s in seed])+' '+str(marker)+'\n')
                    idx += 1
        f.close()
        return
##############################################################################
...
def MultiPolygon_to_Indexed_Polygons(multipol, merge_overlappings=True):
    ...
    iterloop = sorted(multipol.geoms, key=lambda geom: geometry.box(
        *geom.bounds).area, reverse=True) if 'Multi' in multipol.geom_type else [multipol]
    for pol in iterloop:
        if not pol.area:
            done += 1
            continue
        if not pol.is_valid:
            UI.logprint("Invalid polygon detected at",
                        list(pol.exterior.coords)[0])
            done += 1
            continue
...
def coastline_to_MultiPolygon(coastline, lat, lon, custom_source=False):
    ...
    while bdcoords:
        ...
        while next_coord != first_coord:
            ...
            if count == 1000:  # dead loop caused by faulty osm coastline data
                UI.lvprint(
                    1, "ERROR is OSM coastline data, probably caused by a coastline way with wrong orientation.")
                return geometry.MultiPolygon()
        ...
        for coord in remove_coords:
            try:
                ...
                (x, y) = bd_point(coord)
                UI.lvprint(1, "ERROR is OSM coastline data, probably caused by a triple junction around lat=", str(
                    y+lat), " lon=", str(x+lon))
                return geometry.MultiPolygon()
    ...
    inpol = ensure_MultiPolygon(cut_to_tile(ops.cascaded_union(
...
def weighted_normals(way, side='left'):  # normalized in the given metric
    ...
    sign = numpy.array(
        [[-1/scalx, 1]]) if side == 'left' else numpy.array([[1/scalx, -1]])
    tg = way[1:]-way[:-1]
    tg[:, 0] *= scalx
    ...
    if N > 2:
        scale = 1e-6 + \
            numpy.linalg.norm(tg[1:-1]+tg[:-2], axis=1).reshape(N-2, 1)
...
def refine_way(way, max_length):  # max_length assumed in meter
    ...
    for i in range(len(way)-1):
        ...
        ins = int(sqrt(numpy.sum(
            (way[i]-way[i+1])**2*numpy.array([[scalx**2, 1]])))*GEO.lat_to_m//max_length)
        new_way.extend([(j/(ins+1)*way[i+1][0]+(ins+1-j)/(ins+1)*way[i][0], j/(ins+1)
                       * way[i+1][1]+(ins+1-j)/(ins+1)*way[i][1]) for j in range(1, ins+1)])
...
def least_square_fit_altitude_along_way(way, steps, dem, weights=False):
    ...
    if not weights:
        ...
        w = (numpy.maximum(numpy.arange(steps+1),
             steps-numpy.arange(steps+1))+steps//2)**2
...
def weighted_alt(node, alt_idx, alt_dico, dem):
    ...
    for idx in alt_idx.intersection((x-eps1, y-eps1, x+eps1, y+eps1)):
        ...
        alti += numpy.polyval(leastsquarefit,
                              linestring.project(pt, normalized=True))*weight
...
def min_bounding_rectangle(pol):
    ...
    for i in range(len(edges)):
        ...
        (xmin, ymin, xmax, ymax) = affinity.rotate(
            pol, -1*angle, origin=tuple(way[i]), use_radians=True).bounds
