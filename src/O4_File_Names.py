...
def jpeg_file_name_from_attributes(til_x_left, til_y_top, zoomlevel, provider_code):
    if provider_code == 'g2xpl_16':
        ...
        file_name = str(til_y_top)+"_"+str(til_x_left)+"_" + \
            provider_code+str(zoomlevel)+".jpg"
    return file_name
##############################################################################
...
def jpeg_file_dir_from_attributes(lat, lon, zoomlevel, provider):
    if not provider:
        ...
        file_dir = os.path.join(Imagery_dir, short_latlon(
            lat, lon), provider['code']+'_'+str(zoomlevel))
    elif provider['imagery_dir'] == 'grouped':
        file_dir = os.path.join(Imagery_dir, long_latlon(
            lat, lon), provider['code']+'_'+str(zoomlevel))
        ...
        file_dir = os.path.join(
            Imagery_dir, provider['imagery_dir'], provider['code']+'_'+str(zoomlevel))
    return file_dir
##############################################################################
...
def dds_file_name_from_attributes(til_x_left, til_y_top, zoomlevel, provider_code, file_ext='dds'):
    if provider_code == 'g2xpl_16':
        ...
        file_name = str(til_y_top)+"_"+str(til_x_left)+"_" + \
            provider_code+str(zoomlevel)+"."+file_ext
    return file_name
##############################################################################
