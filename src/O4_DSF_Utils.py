...
class QuadTree(dict):
        ...
        if level == 0:
            ...
            for i in range(2**level):
                for j in range(2**level):
                    key = (numpy.binary_repr(i).zfill(level),
    ...
    def statistics(self):
        ...
        UI.vprint(1, "     Average depth:", depths.mean(),
                  ", Average bucket size:", lengths.mean())
...
def zone_list_to_ortho_dico(tile):
    ...
    if tile.cover_airports_with_highres in ['True', 'ICAO']:
        ...
        try:
            ...
            UI.vprint(1, "   WARNING: File", FNAMES.apt_file(
                tile), "is missing (erased after Step 1?), cannot check airport info for upgraded zoomlevel.")
            dico_airports = {}
        if tile.cover_airports_with_highres == 'ICAO':
            airports_list = [
                airport for airport in dico_airports if dico_airports[airport]['key_type'] == 'icao']
        else:
    ...
    i = 1
    base_zone = ((tile.lat, tile.lon, tile.lat, tile.lon+1, tile.lat+1, tile.lon+1,
                 tile.lat+1, tile.lon, tile.lat, tile.lon), tile.default_zl, tile.default_website)
    ...
    for til_x in range(til_x_min, til_x_max+1, 16):
        for til_y in range(til_y_min, til_y_max+1, 16):
            ...
            dico_customzl[(til_x, til_y)] = (
                til_x_text, til_y_text, zoomlevel, provider_code)
    ...
    with open(os.path.join(tile.build_dir, 'terrain', ter_file_name), 'w') as f:
        ...
        if tri_type in (1, 2) and not is_overlay:  # experimental water
            ...
            if not os.path.exists(os.path.join(tile.build_dir, 'textures', 'water_normal_map.dds')):
                shutil.copy(os.path.join(FNAMES.Utils_dir, 'water_normal_map.dds'), os.path.join(
        ...
        # constant transparency level
            ...
            if not os.path.exists(os.path.join(tile.build_dir, 'textures', 'water_transition.png')):
                shutil.copy(os.path.join(FNAMES.Utils_dir, 'water_transition.png'), os.path.join(
                    tile.build_dir, 'textures'))
            ...
            f.write('BORDER_TEX ../textures/'+FNAMES.mask_file(til_x_left,
                    til_y_top, zoomlevel, provider_code)+'\n')
        # dxt5 with normal map
        elif tri_type == 2 and tile.imprint_masks_to_dds and (tile.experimental_water & 2):
            f.write('TEXTURE_NORMAL '+str(2**(17-zoomlevel)) +
                    ' ../textures/water_normal_map.dds\n')
            f.write('GLOBAL_specular 1.0\n')
            f.write('NORMAL_METALNESS\n')
...
def build_dsf(tile, download_queue):
    ...
    dsf_file_name = os.path.join(
        tile.build_dir, 'Earth nav data', FNAMES.long_latlon(tile.lat, tile.lon)+'.dsf')
    ...
    for i in range(nbr_nodes):
        node_coords[5*i:5*i+3] = [float(x)
                                  for x in f_mesh.readline().split()[:3]]
        pool_quadtree.insert(float2qquad(
            node_coords[5*i]-tile.lon), float2qquad(node_coords[5*i+1]-tile.lat), quad_init_level)
    pool_quadtree.clean()
    ...
    for i in range(3):
        f_mesh.readline()
    for i in range(nbr_nodes):
        node_coords[5*i+3:5*i+5] = [float(x)
                                    for x in f_mesh.readline().split()[:2]]
    ...
    for key in pool_quadtree:
        ...
        altitudes = numpy.array([node_coords[5*idx_node+2]
        ...
        node_icoords[[5*idx_node+2 for idx_node in plist]
                     ] = numpy.round((altitudes-altmin)*inv_stp)
        pool_param[key_to_idx_pool[key]] = (scal_x, tile.lon+int(key[0], 2)*scal_x, scal_y, tile.lat+int(
            key[1], 2)*scal_y, scale_z, altmin, 2, -1, 2, -1, 1, 0, 1, 0, 1, 0, 1, 0)
    ...
    for tri in [tri for tri in tri_list if tri[3] == 2]:
        ...
        bary_lat = (node_coords[5*n1+1] +
                    node_coords[5*n2+1]+node_coords[5*n3+1])/3
            ...
            if terrain_attributes not in skipped_terrains_for_masking:
                ...
                if mask_im:
                    ...
                    mask_im.save(os.path.join(
                        tile.build_dir, "textures", FNAMES.mask_file(*texture_attributes)))
                else:
                    skipped_terrains_for_masking.add(terrain_attributes)
                    # clean up potential old masks in the tile dir
                    try:
                        os.remove(os.path.join(tile.build_dir, "textures",
                    ...
                    if (not os.path.isfile(os.path.join(tile.build_dir, 'textures', texture_file_name))) or (tile.imprint_masks_to_dds):
                        if 'g2xpl' not in texture_attributes[3]:
                            ...
                            texture_file_name = texture_file_name.replace(
                                'dds', 'partial.dds')
                            UI.vprint(1, "   Texture file " +
                                      texture_file_name+" already present.")
                        else:
                            UI.vprint(
                                1, "   Missing a required texture, conversion from g2xpl requires texture download.")
                            download_queue.put(texture_attributes)
                    else:
                        UI.vprint(1, "   Texture file " +
                                  texture_file_name+" already present.")
                ...
                if node_hash in textured_nodes:
                    ...
                    if not tile.imprint_masks_to_dds:  # border_tex
                        ...
                        dsf_pools[idx_dsfpool].extend(
                            (int(round(s*65535)), int(round(t*65535)), int(round(s*65535)), int(round(t*65535))))
            ...
            for n in (n1, n3, n2):     # beware of ordering for orientation !
                ...
                if node_hash in textured_nodes:
                    ...
                    if (tile.experimental_water & 2):
                        ...
                        dsf_pools[idx_dsfpool].extend(node_icoords[5*n:5*n+3])
                        dsf_pools[idx_dsfpool].extend((32768, 32768, int(
                            round(s*65535)), int(round(t*65535)), 0, int(round(tile.ratio_water*65535))))
                    len_textured_nodes += 1
                    pos_in_pool = dsf_pool_length[idx_dsfpool]
                    textured_nodes[node_hash] = (idx_dsfpool, pos_in_pool)
                    dsf_pool_length[idx_dsfpool] += 1
    ...
    size_of_defn_atom = 48+len(bTERT)+len(bOBJT) + \
        len(bPOLY)+len(bNETW)+len(bDEMN)
    ...
    for terrain_idx in textured_tris:
        ...
        for idx_dsfpool in textured_tris[terrain_idx]:
            if idx_dsfpool != 'cross-pool':
                ...
                # POOL INDEX
                f.write(struct.pack('<H', dico_new_dsf_pool[idx_dsfpool]))
                ...
                for j in range(blocks):
                    ...
                    for k in range(255):
                        # COORDINATE IDX
                        f.write(struct.pack(
                            '<H', textured_tris[terrain_idx][idx_dsfpool][255*j+k]))
                remaining_tri_p = len(
                ...
                if remaining_tri_p != 0:
                    ...
                    # COORDINATE COUNT
                    f.write(struct.pack('<B', remaining_tri_p))
                    for k in range(remaining_tri_p):
                        # COORDINATE IDX
                        f.write(struct.pack(
                            '<H', textured_tris[terrain_idx][idx_dsfpool][255*blocks+k]))
            else:  # idx_dsfpool == 'cross-pool'
                pool_idx_init = textured_tris[terrain_idx][idx_dsfpool][0]
                # POOL SELECT
                f.write(struct.pack('<B', 1))
                # POOL INDEX
                f.write(struct.pack('<H', dico_new_dsf_pool[pool_idx_init]))
                f.write(struct.pack('<B', 18))    # TERRAIN PATCH FLAGS AND LOD
                f.write(struct.pack('<B', flag))  # FLAG
                f.write(struct.pack('<f', 0))     # NEAR LOD
                f.write(struct.pack('<f', lod))   # FAR LOD
                ...
                for j in range(blocks):
                    # PATCH TRIANGLE CROSS-POOL
                    f.write(struct.pack('<B', 24))
                    f.write(struct.pack('<B', 255))  # COORDINATE COUNT
                    for k in range(255):
                        f.write(struct.pack(
                            '<H', dico_new_dsf_pool[textured_tris[terrain_idx][idx_dsfpool][510*j+2*k]]))    # POOL IDX
                        # POS_IN_POOL IDX
                        f.write(struct.pack(
                            '<H', textured_tris[terrain_idx][idx_dsfpool][510*j+2*k+1]))
                remaining_tri_p = int(
                ...
                if remaining_tri_p != 0:
                    ...
                    for k in range(remaining_tri_p):
                        f.write(struct.pack(
                            '<H', dico_new_dsf_pool[textured_tris[terrain_idx][idx_dsfpool][510*blocks+2*k]]))   # POOL IDX
                        # POS_IN_PO0L IDX
                        f.write(struct.pack(
    ...
    UI.vprint(1, "     DSF file encoded, total size is :",
              size_of_dsf, "bytes", "("+UI.human_print(size_of_dsf)+")")
    return 1
##############################################################################
