    ...
    def update_dicosm(self, osm_input, input_tags=None, target_tags=None):
        ...
        if isinstance(osm_input, str):
            ...
            try:
                ...
                UI.vprint(1, "    Could not open", osm_file_name,
            ...
            if '<node id=' in items[0]:
                ...
                if items[1] != 'way' or role not in ('outer', 'inner'):
                    ...
                    UI.lvprint(2, "Relation id=", osmid, "contains a member of type", "'" +
                               items[1]+"'", "and role", "'"+role+"'", "which was not treated (only deal with 'ways' of role 'inner' or 'outer').")
                    continue
                try:
                ...
                for role, endpt in ((r, e) for r in ['outer', 'inner'] for e in dico_rel_check[r]):
                    if len(dico_rel_check[role][endpt]) != 2:
                        ...
                        break
                if bad_rel == True:
                    UI.lvprint(2, "Relation id=", osmid,
        ...
        if not normal_exit:
            UI.lvprint(
                0, "ERROR: OSM overpass server answer was corrupted (no ending </OSM> tag)")
            return 0
    ...
    def write_to_file(self, filename):
        ...
        if not len(self.dicosmfirst['n']):
            for nodeid, (lonp, latp) in self.dicosmn.items():
                fout.write('  <node id="'+str(nodeid)+'" lat="'+'{:.7f}'.format(
                    latp)+'" lon="'+'{:.7f}'.format(lonp)+'" version="1"/>\n')
        else:
            for nodeid, (lonp, latp) in self.dicosmn.items():
        ...
        for relid in tuple(self.dicosmfirst['r'])+tuple(set(self.dicosmrorig).difference(self.dicosmfirst['r'])):
            ...
            for wayid in self.dicosmrorig[relid]['outer']:
                fout.write('    <member type="way" ref="' +
                           str(wayid)+'" role="outer"/>\n')
            for wayid in self.dicosmrorig[relid]['inner']:
                fout.write('    <member type="way" ref="' +
                           str(wayid)+'" role="inner"/>\n')
...
def OSM_queries_to_OSM_layer(queries, osm_layer, lat, lon, tags_of_interest=[], server_code=None, cached_suffix=''):
    ...
    for query in queries:
        ...
        if UI.red_flag:
            return 0
        if not response:
            UI.logprint("No valid answer for", query, "after",
                        max_osm_tentatives, ", skipping it.")
            UI.vprint(1, "      No valid answer after",
                      max_osm_tentatives, ", skipping it.")
            return 0
...
def OSM_query_to_OSM_layer(query, bbox, osm_layer, tags_of_interest=[], server_code=None, cached_file_name=''):
    ...
    if cached_file_name and os.path.isfile(cached_file_name):
        ...
        if UI.red_flag:
            return 0
        if not response:
            UI.lvprint(1, "      No valid answer for", query,
                       "after", max_osm_tentatives, ", skipping it.")
            return 0
        osm_layer.update_dicosm(response, input_tags, target_tags)
        if cached_file_name:
...
def get_overpass_data(query, bbox, server_code=None):
    ...
    while True:
        ...
        if not server_code:
            true_server_code = random.choice(list(overpass_servers.keys(
            ))) if overpass_server_choice == 'random' else overpass_server_choice
            ...
            if '200' in str(r):
                if b"</osm>" not in r.content[-10:] and b"</OSM>" not in r.content[-10:]:
                    UI.vprint(1, "        OSM server", true_server_code,
                              "sent a corrupted answer (no closing </osm> tag in answer), new tentative in", 2**tentative, "sec...")
                elif len(r.content) <= 1000 and b"error" in r.content:
                    UI.vprint(1, "        OSM server", true_server_code,
                              "sent us an error code for the data (data too big ?), new tentative in", 2**tentative, "sec...")
                else:
                    break
            else:
                UI.vprint(1, "        OSM server", true_server_code,
...
def OSM_to_MultiPolygon(osm_layer, lat, lon, filter=None):
    ...
    if filter:
        ...
        UI.vprint(2, "    Total number of geometries:", len(
            ret_val[0].geoms), len(ret_val[1].geoms))
    else:
        ret_val = geometry.MultiPolygon(multilist)
