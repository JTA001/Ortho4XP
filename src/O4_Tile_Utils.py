...
def build_tile(tile):
    ...
    UI.is_working = 1
    UI.red_flag = False
    UI.logprint("Step 3 for tile lat=", tile.lat,
    ...
    download_thread = threading.Thread(target=download_textures, args=[
                                       tile, download_queue, convert_queue])
    build_dsf_thread.start()
    if not skip_downloads:
        download_thread.start()
        if not skip_converts:
            UI.vprint(1, "-> Opening convert queue and",
                      max_convert_slots, "conversion workers.")
            dico_conv_progress = {'done': 0, 'bar': 3}
            convert_workers = parallel_launch(
                IMG.convert_texture, convert_queue, max_convert_slots, progress=dico_conv_progress)
...
def build_tile_list(tile, list_lat_lon, do_osm, do_mesh, do_mask, do_dsf, do_ovl, do_ptc):
    ...
    UI.red_flag = 0
    timer = time.time()
    UI.lvprint(0, "Batch build launched for a number of",
               len(list_lat_lon), "tiles.")
    k = 0
    for (lat, lon) in list_lat_lon:
        k += 1
        UI.vprint(1, "Dealing with tile ", k, "/", len(list_lat_lon),
        ...
        try:
            ...
            print(e)
    UI.lvprint(0, "Batch process completed in",
